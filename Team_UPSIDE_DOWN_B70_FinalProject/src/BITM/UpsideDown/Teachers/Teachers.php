...
class Teachers extends DB{
    ...
    public function store() {
        ...
        $query="INSERT INTO teachers (`first_name`, `last_name`, `email`,`password`, `profile_picture`,`course`,`email_verified`) 
VALUES (:firstName, :lastName, :email, :password, :profile_pic, :course, :email_token)";
    ...
    public function change_password(){
        $query="UPDATE `e_classroom`.`teachers` SET `password`=:password  WHERE `teachers`.`email` =:email";
        $sth=$this->conn->prepare($query);
    ...
    public function validTokenUpdate(){
        $query="UPDATE `e_classroom`.`teachers` SET  `email_verified`='".'Yes'."' WHERE `users`.`email` ='$this->email'";
        $sth=$this->conn->prepare($query);
        $result = $sth->execute();
